  ...
  <header class="py-4">
    <div class="container m-auto flex items-center justify-between">
      <div class="mx-2 flex items-center">
        ...
        <router-link to="/" class="text-lg font-mono pl-4 font-bold no-underline text-black hover:text-indigo">Sea Solver<span class="hidden md:inline"> - ウミガメのスープを解くサイト -</span></router-link>
      </div>

      <div class="mx-2 flex items-center">
        <img v-tooltip="'マイページ'" class="w-8 h-8 mr-5 rounded-full" @click.prevent="jumpTo('/mypage')" v-lazy="myAvatar" v-if="sigendIn">
        <button v-tooltip="'問題作成'" class="mr-4 text-indigo" @click.prevent="jumpTo('/quizzes/new')" v-if="sigendIn"><font-awesome-icon icon="edit" size="lg" /></button>
        <button v-tooltip="'ログアウト'" class="text-indigo" @click.prevent="signOut()" v-if="sigendIn"><font-awesome-icon icon="sign-out-alt" size="lg" /></button>
        <button @click.prevent="jumpTo('/signin')" class="text-sm mr-2 rounded cursor-pointer
          no-underline bg-green hover:bg-green-dark text-center block p-2 text-white items-center justify-center" v-if="!sigendIn">ログイン</button>
        <button @click.prevent="jumpTo('/signup')" class="text-sm rounded cursor-pointer
          no-underline bg-blue hover:bg-blue-dark text-center block p-2 text-white items-center justify-center" v-if="!sigendIn">新規登録</button>
      </div>
